...
internal object LiveDiscover {
    ...
    fun prepare(activity: MainActivity, bounds: Rect, width: Float) {
        ...
        val stateJson = org.json.JSONObject(activity.getSharedPreferences("launcher", 0).getString("state", "{}") ?: "{}")
        val activeTarget = stateJson.optString("companionFeed").let { str ->
            if (str.isEmpty() && stateJson.optBoolean("googleDiscover", false)) DiscoverClient.GOOGLE_PACKAGE 
            else str 
        }
        ...
        val available = if (activeTarget == DiscoverClient.GOOGLE_PACKAGE) {
            DiscoverClient.isAvailable(activity)
        } else {
            BridgeOverlayClient.isAvailable(activity, activeTarget)
        }
        
        if (!available) {
            message.value = "The selected feed isn't available or installed."
            return
        }
        pageWidth = width
...
class LiveDiscoverActivity : ComponentActivity() {
    private lateinit var client: FeedClient
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val stateJson = org.json.JSONObject(getSharedPreferences("launcher", 0).getString("state", "{}") ?: "{}")
        vertical = stateJson.optBoolean("verticalStatus", true)
        val companionFeed = stateJson.optString("companionFeed").let { str ->
            if (str.isEmpty() && stateJson.optBoolean("googleDiscover", false)) DiscoverClient.GOOGLE_PACKAGE 
            else str 
        }
        ...
        val onState: (String?) -> Unit = { LiveDiscover.message.value = it; if (it != null) frame.hide() }
        val onVisible = frame::reveal
        val onProgress: (Float) -> Unit = { if (LiveDiscover.native(it)) frame.liveProgress = it else frame.invalidate() }
        
        client = if (companionFeed == DiscoverClient.GOOGLE_PACKAGE) {
